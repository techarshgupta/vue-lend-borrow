...
export const colorsOutlineHover = {
  ...
  info: "hover:bg-teal-700 hover:text-white",
};

export const getButtonColor = (type, color, isOutlined, hasHover) => {
  const base = [
    isOutlined ? colorsText[color] : colorsBg[color],
    type === "primary" ? `border ${colorsBorders[color]}` : "",
  ];

  if (hasHover && type === "primary") {
